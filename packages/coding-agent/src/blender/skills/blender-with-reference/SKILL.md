...
4. If the user is framing a comparison manually or referring to the currently selected object or current view, first use the injected live Blender context summary when it is present and clearly usable. Only call `blender_session_context` again when that summary is missing, stale, or too incomplete to make the target and viewpoint explicit.
